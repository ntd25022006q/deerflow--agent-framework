# ═══════════════════════════════════════════════════════════════
# DEERFLOW AGENT FRAMEWORK v1.0 - CONTINUE.ENV ENFORCEMENT
# ═══════════════════════════════════════════════════════════════
# Continue.dev Integration
# Apply these rules to EVERY autocompletion, chat response, and edit
# VIOLATION OF ANY RULE = UNACCEPTABLE OUTPUT
# ═══════════════════════════════════════════════════════════════

## CONTINUE BEHAVIOR DIRECTIVES

When operating within Continue.dev, you MUST:

- Analyze the full context of the current file, open tabs, and project structure before generating code
- Ensure all autocompletions match the surrounding code style, indentation, and patterns exactly
- Provide contextually aware inline completions that integrate seamlessly with existing code
- When responding in chat, always reference specific files, line numbers, and code snippets
- Generate production-ready code with comprehensive error handling and type annotations
- Include test suggestions alongside all feature code generation
- Never generate code that introduces new dependencies without explicit justification
...
describe('UserService') {
  describe('createUser') {
    it('should create a user with valid input and return the created user')
...
## CONTINUE-SPECIFIC DIRECTIVES

### Autocomplete Rules
- Inline completions must match the existing indentation, style, and conventions exactly
- Completions should be immediately usable without manual modification
- Generated code must include proper type annotations matching the project's type system
- Completions must not introduce new imports without the corresponding import statement
- Tab completion suggestions should respect the project's existing patterns

### Chat Mode Rules
- When responding in chat, always reference specific files, line numbers, and code snippets
- Provide multiple approaches when appropriate, with trade-off analysis
- When asked to debug, always read the relevant code first before suggesting fixes
- Include test suggestions alongside all code change recommendations
- Explain the reasoning behind every architectural suggestion

### Context Awareness
- Leverage Continue's context providers to understand the full project structure
- Use the `@file`, `@folder`, and `@codebase` references to build comprehensive context
- Ensure generated code is compatible with all relevant configuration files (tsconfig, eslint, etc.)
- Check for existing patterns in the codebase before proposing new ones
