# ═══════════════════════════════════════════════════════════════
# DEERFLOW AGENT FRAMEWORK v1.0 - OPENAI CODEX ENFORCEMENT
# ═══════════════════════════════════════════════════════════════
# OpenAI Codex Integration
# Apply these rules to EVERY code generation, edit, and response
# VIOLATION OF ANY RULE = UNACCEPTABLE OUTPUT
# ═══════════════════════════════════════════════════════════════

## CODEX BEHAVIOR DIRECTIVES

When operating as OpenAI Codex, you MUST:

- Analyze the full project context before generating any code
- Generate code that integrates seamlessly with existing patterns and conventions
- Provide complete, production-ready implementations — not partial snippets
- Include comprehensive error handling in every generated function
...
11. **NEVER use `eval()`, `Function()`, `innerHTML`, or `dangerouslySetInnerHTML`** unless absolutely necessary AND properly sanitized. These are security vulnerabilities by default.
...
### Responsive Design
- **Mobile-first approach:** Design for smallest screens first, then enhance
- **Breakpoints:** Define a consistent breakpoint system and use it everywhere
...
- Implement ARIA attributes where semantic HTML is insufficient
...
- **Proper loading states:** Skeleton screens, spinners, or progress indicators
- **Error states:** User-friendly error messages with recovery actions
- **Empty states:** Meaningful guidance when no data is available

### Animation
- Respect `prefers-reduced-motion` media query
- Animation duration: 150-300ms for micro-interactions, never exceed 500ms
- Animate only `transform` and `opacity` properties (avoid layout-triggering animations)
...
### Security
- NEVER commit secrets — use `.gitignore`, `git-secrets`, or pre-commit hooks
...
- Generated code must be complete and production-ready — not partial snippets
- All generated functions must include proper type annotations
- Generated code must include comprehensive error handling
- Generated business logic must be accompanied by unit tests
- Code must follow the existing project patterns exactly
- All generated code must be compatible with the project's dependency versions

### Context Awareness
- Analyze the full repository context before generating code
- Match the existing code style, naming conventions, and architectural patterns
- Ensure generated imports use the project's path aliases
- Verify generated code against the project's TypeScript configuration
- Ensure compatibility with the project's build toolchain
